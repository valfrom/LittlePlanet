...
func _rebuild_multimesh() -> void:
    ...
    multimesh.custom_data_format = RenderingServer.MULTIMESH_CUSTOM_DATA_FLOAT
    ...
    for index in instance_count:
        ...
        var color_variation: float = rng.randf()
        var bend_variation: float = rng.randf()
        var phase: float = rng.randf()
        var unused: float = rng.randf()
