...
func _align_camera_to_gravity() -> void:
    var player: Node = get_parent()
    if player == null or not player.has_method("_get_up_direction"):
        return

    var up_dir: Vector3 = player._get_up_direction()
    ...
    if forward.length_squared() < 0.0001:
        forward = -camera_transform.basis.y

    if abs(forward.normalized().dot(up_dir)) > 0.99:
        forward = -camera_transform.basis.x

    camera_base.global_transform = Transform3D(Basis.looking_at(forward, up_dir), camera_transform.origin)
