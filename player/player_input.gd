    ...
    if shooting:
        ...
        var col = get_parent().get_world_3d().direct_space_state.intersect_ray(
            PhysicsRayQueryParameters3D.create(
                ray_from,
                ray_from + ray_dir * 1000,
                0b11,
                Array([self], TYPE_RID, "", null)
            )
        )
        if col.is_empty():
...
func rotate_camera(move: Vector2) -> void:
    ...
    camera_rot.rotation.x = clampf(
        camera_rot.rotation.x + move.y,
        CAMERA_X_ROT_MIN,
        CAMERA_X_ROT_MAX
    )


func align_camera_to_up(up_dir: Vector3) -> void:
    var camera_transform: Transform3D = camera_base.global_transform
    var current_up: Vector3 = camera_transform.basis.y.normalized()
    var axis: Vector3 = current_up.cross(up_dir)
    var dot: float = clampf(current_up.dot(up_dir), -1.0, 1.0)
    ...
    var rotation: Basis = Basis(axis / axis_length, angle)
    camera_transform.basis = rotation * camera_transform.basis
    camera_base.global_transform = camera_transform.orthonormalized()
