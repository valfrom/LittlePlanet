...
func align_camera_to_up(up_dir: Vector3) -> void:
    var current_transform: Transform3D = camera_base.global_transform
    var forward: Vector3 = -get_camera_rotation_basis().z

    if absf(forward.dot(up_dir)) > 0.999:
        forward = -camera_base.global_transform.basis.z

    var aligned_basis: Basis = Basis.looking_at(forward, up_dir).orthonormalized()
    camera_base.global_transform = Transform3D(aligned_basis, current_transform.origin)
