...
func _process(delta: float) -> void:
    ...
    if player:
        _align_camera_to_gravity(player._get_up_direction())
...
func _align_camera_to_gravity(up_dir: Vector3) -> void:
    var camera_transform: Transform3D = camera_base.global_transform
    var forward: Vector3 = -camera_transform.basis.z
    forward = forward - up_dir * forward.dot(up_dir)

    if forward.length_squared() < 0.0001:
        forward = -global_transform.basis.z

    camera_transform.basis = Basis.looking_at(camera_transform.origin + forward.normalized(), up_dir)
    camera_base.global_transform = camera_transform
