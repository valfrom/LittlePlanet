...
func apply_input(delta: float) -> void:
    ...
    camera_x = up_dir.cross(camera_z).normalized()
    ...
    var orientation_basis: Basis = orientation.basis
    orientation_basis.y = up_dir
    orientation_basis.x = up_dir.cross(-orientation_basis.z).normalized()
    orientation_basis.z = -orientation_basis.x.cross(orientation_basis.y).normalized()
    orientation.basis = orientation_basis

    var tangential_motion: Vector3 = orientation.origin - up_dir * orientation.origin.dot(up_dir)
    var tangential_velocity: Vector3 = tangential_motion / delta
