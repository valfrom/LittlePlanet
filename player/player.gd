...
func apply_input(delta: float) -> void:
    ...
    var gravity_strength: float = gravity_vector.length()
...
func _get_gravity_direction() -> Vector3:
    if planet_center != null:
        var direction_to_center: Vector3 = (planet_center.global_position - global_position)
        var distance_squared: float = direction_to_center.length_squared()
        if distance_squared > 0.0:
            return direction_to_center / sqrt(distance_squared)
