...
func apply_input(delta: float) -> void:
    ...
    var camera_z: Vector3 = _project_on_plane(camera_basis.z, up_dir).normalized()
    var camera_x: Vector3 = _project_on_plane(camera_basis.x, up_dir).normalized()
    ...
    if not on_air and player_input.jumping:
        var tangential_velocity: Vector3 = _project_on_plane(velocity, up_dir)
    ...
    var vertical_velocity: float = velocity.dot(up_dir)
    ...
    if on_air:
        ...
        var aim_forward: Vector3 = -player_input.get_camera_rotation_basis().z
        aim_forward = _project_on_plane(aim_forward, up_dir).normalized()
        var q_to: Quaternion = Basis.looking_at(aim_forward, up_dir).get_rotation_quaternion()
    ...
    velocity += _get_gravity(up_dir) * delta
...
func _get_up_direction() -> Vector3:
    ...
    var offset := global_position - planet_center.global_position
...
func _get_gravity(up_dir: Vector3) -> Vector3:
    return -up_dir * gravity_strength
