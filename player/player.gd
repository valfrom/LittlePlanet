...
func apply_input(delta: float) -> void:
    ...
    if planet_center != null:
        ...
        gravity_vec = -up_dir * gravity_strength
    ...
    camera_z = camera_z - up_dir * camera_z.dot(up_dir)
    if camera_z.length_squared() > 0.0:
        camera_z = camera_z.normalized()
    else:
        camera_z = (-orientation.basis.z - up_dir * orientation.basis.z.dot(up_dir)).normalized()

    camera_x = up_dir.cross(camera_z)
    if camera_x.length_squared() > 0.0:
        camera_x = camera_x.normalized()
    else:
        camera_x = up_dir.cross(Vector3.FORWARD).normalized()
    ...
    _align_orientation_to_up(up_dir)
...
func _align_orientation_to_up(up_dir: Vector3) -> void:
    orientation = orientation.orthonormalized()
    orientation.basis.y = up_dir
    orientation.basis.x = up_dir.cross(-orientation.basis.z).normalized()
    orientation.basis.z = -orientation.basis.x.cross(orientation.basis.y).normalized()
